...
auto map_reduce(It p, It q, F1 f1, F2 f2, size_t threads) -> decltype(f2(f1(*p), f1(*p)))
{
    ...
    It next = p, prev;
    std::vector<std::future<TRes>> vecFut;
    for (int i = 0; i < threads - 1; ++i) {
        prev = next;
        std::advance(next, dist);
        vecFut.push_back(std::async(std::launch::async, map_reduce_async<It, F1, F2>, prev, next, f1, f2));        
    }
    vecFut.push_back(std::async(std::launch::async, map_reduce_async<It, F1, F2>, next, q, f1, f2));
    auto result = vecFut[0].get();
    for (int i = 1; i < vecFut.size(); ++i)
        result = f2(result, vecFut[i].get());
    return result;
}
int main()
{
